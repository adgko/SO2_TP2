...
int main(int32_t arg, char *argv[]){ 	/// entra Radio, Contraste K, Brillo L y cantidad de hilos
    ...
    printf("%sbrillo: %d %s\n",KBLU,L,KNRM );
    printf("%sCantidad de hilos: %d %s\n",KBLU,num_threads,KNRM );
	...
	//recorrido de la matriz    
		...
		for(int32_t i=0; i<alto; i++){
			for(int32_t j=id; j<ancho; j=j+nthrds){
				/// cálculo del centro
